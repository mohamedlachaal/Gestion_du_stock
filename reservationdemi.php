<?php
    ...
    if(isset($_POST["date"]))
    {
        ...
        if($errdate==""&&$errdatef=="")
        {
            ...
            $idai=$_GET["mod"];
            require("db.php");
            $query="insert into reservationdemi values(NULL,:ide,:date,:cause,:idr)";
            $stmt=$conn->prepare($query);
            $stmt->execute(array(":ide"=>$idpa,":date"=>$date,":cause"=>$datef,":idr"=>$idai));
...
<div id="container">
            ...
            <form action="#" method="POST">
                ...
                <label><b>la date de demission :</b></label><br>
                <input type="date" placeholder="entrer l'email" name="date" required >
                <br><label><b>cause du demission :</b></label><br>
                <input type="text" placeholder="cause du demission" name="datef" required >
