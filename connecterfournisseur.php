...
if(isset($_POST["email"]))
{
    ...
    if($erremail==""&&$errpaa=="")
    {
            ...
            require("db.php");
            $query="select * from fournisseur where email=:email and password=:password";
            ...
            else
            {
                ...
                $_SESSION["societe"]=$data[0]["societe"];
                $so=$_SESSION["societe"];
                if($so=="coca"){

                    header("location:fournisseurcoca.php");
                }
               
                 if($so=="lait")
                {
                    header("location:fournisseurlait.php");
                }
                 if($so=="fer")
                {
                    header("location:fournisseurfer.php");  
                }
